...
const LAOKA_FORM_UI = {
  ...
  emptyDraft() {
    return {
      name: "",
      photo: "🍽️",
      // Photo réelle importée par l'utilisateur, distincte de `photo` (emoji d'icône). La donnée
      // elle-même (data URL) n'est JAMAIS stockée sur la recette ni dans localStorage — elle vit
      // dans IndexedDB via PHOTO_STORE (app.js), car localStorage a un quota trop bas pour des
      // images (voir historique : erreurs de sauvegarde dès l'ajout d'une photo). `photoImage` ici
      // n'est qu'un aperçu local en mémoire pendant l'édition du formulaire ; `photoImageId` est
      // la référence stable persistée sur la recette (recipe.photoImageId), utilisée pour relire
      // la photo depuis PHOTO_STORE.get() à l'affichage (toujours async, voir ui.js).
      photoImage: null,
      photoImageId: null,
      ...
      time: 30,                      // durée de préparation/cuisson en minutes, affichée sur les cartes et la fiche détail
      notes: "",
  ...
  startEdit(recipe, onDone) {
    this.draft = {
      ...
      photoImage: null,
      photoImageId: recipe.photoImageId || null,
    ...
    // Aperçu photo : chargé de façon async depuis PHOTO_STORE (IndexedDB) après le premier rendu du
    // formulaire, puisque la donnée n'est plus disponible de façon synchrone (voir photoImageId).
    if (this.draft.photoImageId) {
      PHOTO_STORE.get(this.draft.photoImageId).then((dataUrl) => {
        if (!dataUrl || this.draft.photoImageId !== recipe.photoImageId) return; // draft changé entretemps
        this.draft.photoImage = dataUrl;
        const preview = document.getElementById("compPhotoPreview");
        const wrap = document.getElementById("compPhotoPreviewWrap");
        if (preview && wrap) {
          preview.src = dataUrl;
  ...
  renderComposition() {
    ...
    this.screenShell(this.editingRecipeId ? "Modifier ny laoka" : "Ajouter un laoka", `
      <form id="compositionForm" class="form fade-in">
        ...
        <div class="field">
          <label for="compTime">Fotoana ilaina (minitra)</label>
          <input type="number" id="compTime" min="1" step="1" value="${d.time}" required>
        </div>
    ...
    // Import de photo : stockée dans IndexedDB (PHOTO_STORE, app.js) plutôt que sur la recette
    // elle-même, donc plus de limite pratique de taille — le seul plafond ici est la résolution
    // (4000px de côté max), juste pour éviter d'afficher/manipuler des photos absurdement grandes
    // (ex. 12000×9000), sans recompresser la qualité de l'image.
      ...
      reader.onload = () => {
        ...
        img.onload = () => {
          ...
          if (scale < 1) {
            ...
            dataUrl = reader.result; // déjà dans une résolution raisonnable, aucune retouche
          }
          d.photoImage = dataUrl;
          const preview = document.getElementById("compPhotoPreview");
          preview.src = dataUrl;
          document.getElementById("compPhotoPreviewWrap").classList.remove("hidden");
          // Sauvegarde immédiate dans IndexedDB — remplace l'ancienne photo de ce brouillon si elle
          // existait (évite d'accumuler des photos orphelines à chaque changement dans le formulaire).
          const previousId = d.photoImageId;
          PHOTO_STORE.save(dataUrl).then((id) => {
            d.photoImageId = id;
            if (previousId && previousId !== id) PHOTO_STORE.delete(previousId);
          });
        };
    ...
    document.getElementById("compPhotoRemove").addEventListener("click", () => {
      d.photoImage = null;
      if (d.photoImageId) PHOTO_STORE.delete(d.photoImageId);
      d.photoImageId = null;
    ...
    document.getElementById("compositionForm").addEventListener("submit", (e) => {
      ...
      const servings = 1;
      const time = Math.max(1, parseInt(document.getElementById("compTime").value, 10) || 30);
      ...
      this.draft = { name, photo: d.photo || "🍽️", photoImageId: d.photoImageId || null, productIds, type, moment, sauceProductIds, servings, time, notes: d.notes || "", dishSubCategoryOverride: d.dishSubCategoryOverride || null, isLasary: document.getElementById("compIsLasary").checked };
  ...
  saveDraftAsRecipe() {
    ...
    const recipeData = {
      name: d.name,
      photo: d.photo || "🍽️",
      photoImageId: d.photoImageId || null,
      type: d.type,
    ...
    if (this.editingRecipeId) {
      ...
      if (!saved) {
        UI.toast("⚠️ Tsy voatahiry ny fanovana — angamba feno ny fitehirizana.");
        return;
      }
    ...
    const saved = DATA.saveData();
    if (!saved) {
      // Retire la recette de la liste en mémoire pour ne pas laisser l'app dans un état incohérent
      // (recette visible à l'écran mais jamais persistée, qui disparaîtrait au prochain chargement).
      const list = legacyDishCategory === "ro_mazava" ? DATA.state.roMazavaRecipes : DATA.state.recipes;
      const idx = list.findIndex((r) => r.id === newRecipe.id);
      if (idx !== -1) list.splice(idx, 1);
      UI.toast("⚠️ Tsy voatahiry ny laoka — angamba feno ny fitehirizana.");
      return;
    }
