...
const LAOKA_FORM_UI = {
  ...
  renderComposition() {
    ...
    document.getElementById("compositionForm").addEventListener("submit", (e) => {
      ...
      // Vérification stricte des incompatibilités AVANT l'enregistrement.
      // Une combinaison interdite ne doit jamais devenir une recette enregistrée.
      const selectedProducts = productIds.map((id) => DATA.getProduct(id)).filter(Boolean);
      const selectedIds = new Set(productIds);
      const hardConflicts = [];
      selectedProducts.forEach((p) => {
        const incompatibleIds = new Set(Array.isArray(p.incompatibleWith?.products)
          ? p.incompatibleWith.products.filter((x) => (x.percentage ?? 100) >= 100).map((x) => x.productId)
          : []);
        if (Array.isArray(p.incompatibleWith?.productIds)) {
          p.incompatibleWith.productIds.forEach((id) => incompatibleIds.add(id));
        }
        selectedIds.forEach((id) => {
          if (id !== p.id && incompatibleIds.has(id)) hardConflicts.push([p.name || p.id, DATA.getProduct(id)?.name || id]);
        });
      });
      if (hardConflicts.length) {
        const unique = [...new Set(hardConflicts.map((x) => `${x[0]} + ${x[1]}`))];
        errEl.textContent = `Akora tsy mifanaraka: ${unique.join("; ")}. Esory ny iray amin'ireo akora.`;
