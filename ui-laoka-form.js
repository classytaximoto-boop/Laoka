...
const LAOKA_FORM_UI = {
  ...
  renderComposition() {
    ...
    // Import de photo : redimensionnée (max 800px de côté) et recompressée en JPEG qualité 0.7 via
    // canvas avant stockage dans d.photoImage — une photo de téléphone moderne (plusieurs Mo) peut
    // à elle seule dépasser le quota localStorage une fois encodée en base64 ; cette compression
    // ramène généralement le fichier à quelques dizaines de Ko.
    document.getElementById("compPhotoInput").addEventListener("change", (e) => {
      ...
      reader.onload = () => {
        ...
        img.onload = () => {
          ...
          d.photoImage = canvas.toDataURL("image/jpeg", 0.7);
    ...
    document.getElementById("compositionForm").addEventListener("submit", (e) => {
      ...
      this.draft = { name, photo: d.photo || "🍽️", photoImage: d.photoImage || null, productIds, type, moment, sauceProductIds, servings, time, notes: d.notes || "", dishSubCategoryOverride: d.dishSubCategoryOverride || null, isLasary: document.getElementById("compIsLasary").checked };
  ...
  saveDraftAsRecipe() {
    ...
    if (this.editingRecipeId) {
      ...
      if (!saved) {
        UI.toast("⚠️ Tsy voatahiry ny fanovana — angamba feno ny fitehirizana (sary lehibe loatra ?).");
        return;
      }
    ...
    if (!saved) {
      ...
      UI.toast("⚠️ Tsy voatahiry ny laoka — angamba feno ny fitehirizana (sary lehibe loatra ?).");
      return;
    }
