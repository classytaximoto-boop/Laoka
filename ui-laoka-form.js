...
const LAOKA_FORM_UI = {
  ...
  step: 0,
  draft: null,
  /* Contexte d'affichage : "page" (défaut, Paramètres > Ajouter un laoka, plein écran dans
     #profilContent) ou "embedded" (tiroir dépliable dans un autre écran, ex: le modal
     "Ajouter un produit" de Laoka Admin). En mode "embedded", mountEl/onExit/onSaved sont
     fournis par l'appelant : on n'écrit jamais dans #profilContent ni ne navigue vers
     PROFIL_UI dans ce cas. */
  context: "page",
  mountEl: null,
  onExit: null,
  onSaved: null,
  ...
  emptyDraft() {
    return {
      ...
      bouillonCubeItems: [],                  // [{ id, name }] issus de BOUILLON_CUBE_REFERENCE — legumes/anana uniquement

      // 8. Compatibilité en %
      incompatibleProducts: [],                // [{ productId, percentage }]
      incompatibleTypes: [],                    // [{ typeId, percentage }] (typeId = valeur DISH_CATEGORIES — groupe/sokajy, ex: voamaina, pas un laoka individuel)
  ...
  /** Entrée alternative : lance le wizard détaillé embarqué dans un conteneur fourni par
   *  l'appelant (tiroir "Détails laoka (13 étapes)" du modal "Ajouter un produit"), en
   *  sautant l'écran de choix de mode (toujours "detaille" dans ce contexte).
   *  mountEl : élément DOM où injecter chaque étape.
   *  onExit() : appelé quand l'utilisateur referme le tiroir sans enregistrer.
   *  onSaved(recipe) : appelé après enregistrement réussi, à la place de la navigation Profil. */
  renderEmbedded(mountEl, { onExit, onSaved, prefill } = {}) {
    this.context = "embedded";
    this.mountEl = mountEl;
    this.onExit = onExit || null;
    this.onSaved = onSaved || null;
    this.mode = "detaille";
    this.draft = Object.assign(this.emptyDraft(), prefill || {});
    this.step = 0;
    this.renderStep();
  },
  ...
  screenShell(title, innerHTML) {
    if (this.context === "embedded") {
      // Pas d'écran plein-page ni de bouton "Retour" global en mode embarqué : le tiroir
      // gère sa propre fermeture (voir wizardShell ci-dessous pour la variante avec étapes).
      this.mountEl.innerHTML = `<div class="page-header fade-in" style="margin:0 0 10px;"><h3 style="margin:0;">${title}</h3></div>${innerHTML}`;
      return;
    }
  ...
  /* Étape 3 (animal) est conditionnelle : uniquement pertinente si ingredientType = hena/vorona/
     hazan_drano/hazandranomasina/fruits_de_mer/ambany_tanana (audit prompt 1/4 : vorona et
     fruits_de_mer sont désormais des groupes distincts qui restent des animaux, donc l'étape
     animal reste pertinente pour eux). On la saute automatiquement sinon, dans les deux sens
     (Suivant ET Précédent), pour ne jamais bloquer l'utilisateur. */
  isAnimalStepRelevant() {
    const d = this.draft;
    return ["hena", "vorona", "hazan_drano", "hazandranomasina", "fruits_de_mer", "ambany_tanana"].includes(d.ingredientType) || d.dishSubCategory === "ambany_tanana";
  },

  /* RO MATSATSO — règle absolue (prompt 2/4) : anana + eau uniquement, sans sel, huile, viande,
     poisson, épices, autre légume ni condiment ajouté. On saute donc Fangarony/Sauce-épices, qui
     n'ont aucun sens pour ce plat (au lieu de les afficher puis filtrer silencieusement à la fin) —
     même pattern que isAnimalStepRelevant, dans les deux sens. */
  isFangaronyStepRelevant() {
    return this.draft.dishSubCategory !== "ro_matsatso";
  },
  isSauceEpicesStepRelevant() {
    return this.draft.dishSubCategory !== "ro_matsatso";
  },
  ...
  nextStep() {
    ...
    if (this.STEPS[this.step] === "fangarony" && !this.isFangaronyStepRelevant()) this.step += 1;
    if (this.STEPS[this.step] === "sauce_epices" && !this.isSauceEpicesStepRelevant()) this.step += 1;
    this.renderStep();
  },
  prevStep() {
    this.step -= 1;
  ...
  wizardShell(title, bodyHTML, opts = {}) {
    ...
    const closeBtnHTML = this.context === "embedded" ? `<button type="button" class="btn-back" id="laokaWizClose">✕ Fermer ce tiroir</button>` : "";
    const shellHTML = `
      ${closeBtnHTML}
    ...
    `;
    if (this.context === "embedded") {
      this.mountEl.innerHTML = shellHTML;
      const closeBtn = document.getElementById("laokaWizClose");
      if (closeBtn) closeBtn.addEventListener("click", () => { if (this.onExit) this.onExit(); });
    } else {
      const el = document.getElementById("profilContent");
      el.innerHTML = `<button class="btn-back" id="laokaFormBack">← Retour</button>` + shellHTML;
  ...
  stepSauceEpices() {
    const d = this.draft;
    // Cube bouillon (Jumbo/Maggi) : uniquement pour légumes/anana — voir BOUILLON_CUBE_ALLOWED_INGREDIENT_TYPES
    // (ces plats n'ont pas la richesse naturelle de la viande/volaille/poisson, contrairement aux
    // laoka hena/vorona/hazandrano/hazandranomasina/fruits de mer déjà "délicieux" sans exhausteur).
    const showBouillonCube = typeof BOUILLON_CUBE_ALLOWED_INGREDIENT_TYPES !== "undefined"
      && BOUILLON_CUBE_ALLOWED_INGREDIENT_TYPES.includes(d.ingredientType);
    this.wizardShell("7 — Ro / zava-manitra", `
      ...
      ${showBouillonCube ? `
      <div class="field">
        <label>Cube bouillon (safidio raha ilaina)</label>
        <div class="choice-grid" id="wBouillonCubeRef">
          ${BOUILLON_CUBE_REFERENCE.map((c) => `<button type="button" class="choice-chip ${(d.bouillonCubeItems||[]).some(x=>x.id===c.id)?"active":""}" data-id="${c.id}" data-name="${c.name}">${c.name}</button>`).join("")}
        </div>
        <p class="muted small">Ho an'ny legioma sy anana ihany — tsy ilaina amin'ny hena/vorona/trondro efa matsiro.</p>
      </div>` : ""}
    `);
    document.querySelectorAll("#wSauceRef .choice-chip, #wSpiceRef .choice-chip, #wBouillonCubeRef .choice-chip").forEach((btn) => {
      btn.addEventListener("click", () => btn.classList.toggle("active"));
    });
    document.getElementById("wizardStepForm").addEventListener("submit", (e) => {
      e.preventDefault();
      ...
      this.draft.bouillonCubeItems = showBouillonCube
        ? [...document.querySelectorAll("#wBouillonCubeRef .choice-chip.active")].map((b) => ({ id: b.dataset.id, name: b.dataset.name }))
        : [];
      this.nextStep();
    });
  },
  ...
  stepCompatibilite() {
    ...
    this.wizardShell("8 — Tsy mifanaraka amin'inona (%)", `
      ...
      <div class="field">
        <label>Sokajy tsy mifanaraka (voamaina, voalena, rony, sns.) — tsy karazana laoka tsirairay</label>
    ...
    document.getElementById("wAddIncType").addEventListener("click", () => {
      ...
      const idx = list.children.length;
      list.insertAdjacentHTML("beforeend", this.incompatibleTypeRowHTML({ typeId: DISH_CATEGORIES[0], percentage: 100 }, idx));
    ...
    return `
      <div class="field-row inc-row" data-idx="${idx}">
        <select class="wIncTypeSelect">
          ${DISH_CATEGORIES.map((c) => `<option value="${c}" ${it.typeId===c?"selected":""}>${DISH_CATEGORY_LABELS[c]}</option>`).join("")}
        </select>
  ...
  stepRecette() {
    const d = this.draft;
    const products = d.dishSubCategory === "ro_matsatso"
      ? DATA.state.products.filter((p) => typeof ROMATSATSO_RULE !== "undefined" && ROMATSATSO_RULE.allowedAnanaProductIds.includes(p.id))
      : DATA.state.products;
    this.wizardShell("12 — Ny fomba fanaovana", `
      ${d.dishSubCategory === "ro_matsatso" ? `<p class="muted small">⚠️ Ro matsatso : anana + rano fotsiny ihany — tsy misy sira, menaka, hena, trondro na zava-manitra.</p>` : ""}
      <div class="field-row">
        <div class="field">
      ...
      <div class="field">
        ...
        ${d.dishSubCategory === "ro_matsatso" ? `<p class="muted small">Voafetra ho amin'ny karazana anana ihany ny lisitra eto ambany.</p>` : ""}
  ...
  stepRecap() {
    ...
    this.wizardShell("13 — Hamarina", `
      <div class="card">
        ...
        ${(d.bouillonCubeItems || []).length ? `<p>Cube bouillon : ${d.bouillonCubeItems.map((c) => c.name).join(", ")}</p>` : ""}
        <p>Tsy mifanaraka : ${[
          ...d.incompatibleProducts.map((ip) => `${(DATA.getProduct(ip.productId) || {}).name || ip.productId} (${ip.percentage}%)`),
          ...d.incompatibleTypes.map((it) => `${DISH_CATEGORY_LABELS[it.typeId] || it.typeId} (${it.percentage}%)`),
  ...
  saveDraftAsRecipe() {
    ...
    // ro_matsatso range dans le même écran/bucket "ro_mazava" (Paramètres > Ro mazava) — il reste
    // distingué par dishSubCategory="ro_matsatso" pour que ROMATSATSO_RULE s'applique (prompt 2/4).
    else if (d.dishSubCategory === "ro_matsatso") legacyDishCategory = "ro_mazava";
    else if (d.dishSubCategory === "ambany_tanana") legacyDishCategory = "ambany_tanana";

    // RO MATSATSO — règle absolue (prompt 2/4) : on n'enregistre JAMAIS un ro_matsatso avec un
    // ingrédient hors anana. On filtre silencieusement les ingrédients non autorisés plutôt que de
    // bloquer l'enregistrement (l'utilisateur voit alors une recette qui ne contient que l'anana).
    let finalIngredients = d.recipeIngredients.length ? d.recipeIngredients : [];
    if (d.dishSubCategory === "ro_matsatso" && typeof ROMATSATSO_RULE !== "undefined") {
      finalIngredients = finalIngredients.filter((ing) => ROMATSATSO_RULE.allowedAnanaProductIds.includes(ing.productId));
    }
    // CUBE BOUILLON — n'ajoute un coût réel que si le champ a été rempli (donc déjà restreint à
    // legumes/anana par l'affichage conditionnel de stepSauceEpices) ET jamais sur un ro_matsatso
    // (règle absolue : "sans sel ni autre ajout" — un cube contient du sel/exhausteurs).
    const finalBouillonCubeItems = d.dishSubCategory === "ro_matsatso" ? [] : (d.bouillonCubeItems || []);
    if (finalBouillonCubeItems.length) {
      finalBouillonCubeItems.forEach((c) => {
        finalIngredients = finalIngredients.concat([{ productId: c.id, qty: 1, unit: "piece" }]);
      });
    }
    ...
    const newRecipe = {
      ...
      season: d.season,
      // RO MATSATSO — règle absolue : ni fangarony, ni sauce, ni épices (sel compris) ne sont
      // jamais enregistrés sur un ro_matsatso, quoi que le formulaire ait pu proposer avant ce point.
      fangarony: d.dishSubCategory === "ro_matsatso" ? [] : d.fangarony,
      sauceItems: d.dishSubCategory === "ro_matsatso" ? [] : d.sauceItems,
      spiceItems: d.dishSubCategory === "ro_matsatso" ? [] : d.spiceItems,
      sauce: d.dishSubCategory === "ro_matsatso" ? "" : d.sauce,
      spices: d.dishSubCategory === "ro_matsatso" ? "" : d.spices,
      bouillonCubeItems: finalBouillonCubeItems,
      incompatibleWith: {
      ...
      notes: d.notes || "",
      manualPrice: finalIngredients.length ? null : d.price,
    ...
    if (this.context === "embedded") {
      if (this.onSaved) this.onSaved(newRecipe);
    } else {
