...
const RO_MAZAVA_UI = {
  render() {
    ...
    const startDraft = (dishSubCategory) => {
      ...
      LAOKA_FORM_UI.editingRecipeId = null; // s'assure qu'un ancien mode édition ne fuit pas ici
      LAOKA_FORM_UI.draft.dishSubCategoryOverride = dishSubCategory;
      LAOKA_FORM_UI.draft.type = "RONY";
      if (dishSubCategory === "ro_matsatso") {
        // Pré-coche directement l'anana comme seul produit — l'écran de composition reste
        // utilisable pour changer la variété d'anana cochée.
        LAOKA_FORM_UI.draft.productIds = ["anana"];
      }
      // Retour personnalisé : après sauvegarde, on revient à cet écran plutôt qu'à Paramètres
      ...
      PROFIL_UI.render();
      LAOKA_FORM_UI.renderComposition();
    };
