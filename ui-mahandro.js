...
const MAHANDRO_UI = {
      ...
      <div class="mahandro-slide fade-in">
        ...
        ${step.seconds ? `
          <div class="mahandro-timer">
            ...
            <div class="mahandro-timer-target">Fotoana voatondro : ${this.formatDuration(step.seconds)}</div>
    ...
    if (this.timerRunning) {
      ...
      this.timerInterval = setInterval(() => {
        ...
        if (this.timerRemaining >= 0) {
          if (display) {
            display.textContent = this.formatDuration(this.timerRemaining);
            display.classList.remove("mahandro-timer-done");
          }
          if (this.timerRemaining === 0) {
            UI.toast(`⏰ Vita ny fotoana voatondro — dingana ${this.current + 1}! (mbola mandeha ny ora raha mila fanampiny)`);
            if (display) display.classList.add("mahandro-timer-done");
          }
        } else {
          // Dépassement : le chrono continue de tourner et affiche combien de temps a été dépassé
          // par rapport à la durée voulue pour cette étape, sans jamais s'arrêter tout seul —
          // l'utilisateur arrête lui-même quand la cuisson lui semble terminée.
          const overrun = -this.timerRemaining;
          if (display) {
            display.textContent = `+${this.formatDuration(overrun)}`;
            display.classList.add("mahandro-timer-overrun");
          }
        }
      }, 1000);
    }
  ...
  resetTimer() {
    ...
    if (display) {
      ...
      display.classList.remove("mahandro-timer-done");
      display.classList.remove("mahandro-timer-overrun");
    }
    const btn = document.getElementById("mahandroTimerToggle");
