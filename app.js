...
/* ============================================================
   SECTION: PHOTO_STORE — photos de laoka en IndexedDB
   ============================================================
   Les photos importées (ui-laoka-form.js) ne sont PAS stockées dans DATA.state / localStorage —
   celui-ci reste synchrone et a un quota bas (5-10 Mo typique) qui plantait dès qu'une ou deux
   photos étaient ajoutées, même compressées (voir historique : erreur "Tsy voatahiry ny laoka").
   IndexedDB a un quota bien plus large (des centaines de Mo, propre à ce type de données binaires)
   et convient nativement au stockage de blobs/data URLs volumineux.
   Une recette référence sa photo via `recipe.photoImageId` (string), jamais la donnée elle-même —
   la donnée réelle est lue/écrite ici, toujours de façon asynchrone (Promise). */
const PHOTO_STORE = {
  DB_NAME: "laoka_photos",
  STORE_NAME: "photos",
  _dbPromise: null,

  /** Ouvre (ou crée) la base IndexedDB. Mise en cache dans _dbPromise pour ne l'ouvrir qu'une fois. */
  _openDb() {
  ...
  /** Enregistre une photo (data URL, quelle que soit sa taille) sous un nouvel id généré, renvoie
   *  cet id — à stocker dans recipe.photoImageId. */
  async save(dataUrl) {
    const db = await this._openDb();
    const id = "photo_" + Date.now() + "_" + Math.random().toString(36).slice(2, 8);
    return new Promise((resolve, reject) => {
      const tx = db.transaction(this.STORE_NAME, "readwrite");
      tx.objectStore(this.STORE_NAME).put(dataUrl, id);
      tx.oncomplete = () => resolve(id);
      tx.onerror = () => reject(tx.error);
    });
  },

  /** Relit une photo par son id. Renvoie null si absente (id inconnu, ou base pas encore prête). */
  async get(id) {
    if (!id) return null;
    try {
      const db = await this._openDb();
      ...
      console.error("PHOTO_STORE.get a échoué :", err);
      return null;
    }
  },

  /** Supprime une photo (ex: remplacée par une nouvelle, ou retirée du formulaire). Best-effort —
   *  ne bloque jamais l'appelant en cas d'échec (une photo orpheline n'est pas grave). */
  async delete(id) {
    if (!id) return;
    try {
      const db = await this._openDb();
      ...
      console.error("PHOTO_STORE.delete a échoué :", err);
    }
  },
};
...
const DATA = {
  ...
  loadData() {
    ...
    if (raw) {
      try {
        ...
        // Programme de laoka sur 7 jours (Fandaharana herinandro) : inactif par défaut tant que
        // l'utilisateur n'a pas cliqué sur "Activer" depuis l'accueil (voir MENU_PLANNER).
        this.state.mealPlan = this.state.mealPlan || { active: false, weekKey: null, days: {} };
   ...
   *  supposer que la sauvegarde a réussi. Les photos de laoka ne sont PAS dans this.state (voir
   *  PHOTO_STORE, IndexedDB) — un échec ici vient d'ailleurs (volume de recettes/produits/historique
   *  accumulé, ou quota déjà réduit par le navigateur/l'appareil). */
  saveData() {
    try {
  ...
  resetToDemo(persist = true) {
    this.state = {
      ...
      mealPlan: { active: false, weekKey: null, days: {} },
    };
    this.migrateArchitecture();
...
/* ============================================================
   MENU_PLANNER — Programme de laoka sur 7 jours (Fandaharana herinandro)
   ============================================================
   Génère et synchronise automatiquement un programme hebdomadaire (Midi + Soir, 7 jours) à partir
   des recettes réelles de l'utilisateur (DATA.state.recipes / DATA.state.roMazavaRecipes), selon
   les règles suivantes :
     - Alatsinainy (Lundi) à Zoma (Vendredi) : rotation parmi RONY / RITRA / ENDASINA / KETSAKETSA.
     - Alatsinainy (Lundi) : RONY obligatoire, midi et soir.
     - Sabotsy (Samedi) : RONY obligatoire, midi et soir, SANS VIANDE (ni hena/vorona/hazan_drano/
       hazandranomasina/fruits_de_mer — légumes/anana/voamaina/atody uniquement).
     - Alahady (Dimanche) : repas de fête — viande, poisson, volaille ou mix, aucune contrainte de
       mode de cuisson (le meilleur/plus riche disponible).
     - RITRA à base de viande/poisson/fruits de mer -> toujours accompagné d'un Ro mazava.
     - ENDASINA (quel que soit l'ingrédient) -> toujours accompagné d'un Ro mazava OU d'un Lasary
       (achard).
   Le programme est stocké dans DATA.state.mealPlan et persiste (localStorage, comme le reste de
   l'état). Il se régénère automatiquement dès que la semaine ISO courante diffère de celle du
   programme stocké (voir MENU_PLANNER.needsRefresh), ou manuellement via MENU_PLANNER.generate().
   N'invente jamais de recette : pioche exclusivement dans les recettes déjà existantes de
   l'utilisateur ; un créneau reste vide (null) si aucune recette ne correspond aux critères plutôt
   que de forcer un mauvais choix. */
const MENU_PLANNER = {
  DAYS: ["alatsinainy", "talata", "alarobia", "alakamisy", "zoma", "sabotsy", "alahady"],
  DAY_LABELS: {
    alatsinainy: "Alatsinainy", talata: "Talata", alarobia: "Alarobia", alakamisy: "Alakamisy",
    zoma: "Zoma", sabotsy: "Sabotsy", alahady: "Alahady",
  },
  MOMENTS: ["atoandro", "hariva"],
  MOMENT_LABELS: { atoandro: "Atoandro (Midi)", hariva: "Hariva (Soir)" },
  ROTATION_MODES: ["RONY", "RITRA", "ENDASINA", "KETSAKETSA"],
  MEAT_TYPES: ["hena", "vorona", "hazan_drano", "hazandranomasina", "fruits_de_mer"],

  /** Numéro de semaine ISO (ex: "2026-W35"), utilisé pour savoir si le programme stocké correspond
   *  encore à la semaine en cours. */
  ...
  /** true si aucun programme n'existe encore, ou si la semaine ISO a changé depuis la dernière
   *  génération — c'est la condition de régénération automatique ("change chaque semaine"). */
  needsRefresh() {
    const plan = DATA.state.mealPlan;
    if (!plan || !plan.active) return false; // jamais de régénération auto tant que l'utilisateur n'a pas activé le programme
    return plan.weekKey !== this.currentWeekKey();
  },

  /** Active le programme (si pas déjà actif) et (re)génère la semaine courante. */
  activate() {
    if (!DATA.state.mealPlan) DATA.state.mealPlan = { active: false, weekKey: null, days: {} };
    DATA.state.mealPlan.active = true;
    this.generate();
  },

  deactivate() {
    if (!DATA.state.mealPlan) return;
    DATA.state.mealPlan.active = false;
    DATA.saveData();
  },

  /** true si une recette est "sans viande" au sens de la règle Sabotsy : ni viande, ni volaille,
   *  ni poisson (eau douce ou mer), ni fruits de mer. Légumes/anana/voamaina/atody/ambany_tanana
   *  restent autorisés (ambany_tanana n'est pas listé dans MEAT_TYPES : ce sont des abats, pas de
   *  la "viande" au sens culinaire courant de cette règle — reste néanmoins rarement choisi ici
   *  faute d'appartenir à la rotation RONY habituelle). */
  isMeatFree(recipe) {
    if (this.MEAT_TYPES.includes(recipe.ingredientType)) return false;
    // Filet de sécurité si ingredientType est absent/imprécis sur une vieille recette : on regarde
    // aussi les ingrédients eux-mêmes via animalSource (viande/poisson/volaille toujours marqués).
    const hasAnimalIngredient = (recipe.ingredients || []).some((ing) => {
      const p = DATA.getProduct(ing.productId);
      return p && p.animalSource && p.animalSource !== "autre";
    });
    return !hasAnimalIngredient;
  },

  /** true si la recette est à base de viande/poisson/volaille/fruits de mer (mix compris). */
  isMeatBased(recipe) {
    return !this.isMeatFree(recipe);
  },

  /** Recettes candidates pour un mode de cuisson donné (valeur de DISH_COOKING_MODES), parmi les
   *  vraies recettes de l'utilisateur. cookingMethod (alias moderne) ou type (champ historique). */
  recipesForMode(mode) {
    return (DATA.state.recipes || []).filter((r) => (r.cookingMethod || r.type) === mode && r.dishCategory !== "achard");
  },

  /** Choisit une recette au hasard dans une liste, en évitant si possible `avoidIds` (déjà utilisés
   *  cette semaine) — ne filtre que si ça laisse au moins un candidat, pour ne jamais renvoyer null
   *  quand une seule recette existe dans une catégorie donnée. */
  pickRecipe(list, avoidIds) {
    if (!list.length) return null;
    const fresh = list.filter((r) => !avoidIds.has(r.id));
    const pool = fresh.length ? fresh : list;
    return pool[Math.floor(Math.random() * pool.length)];
  },

  /** Un Ro mazava au hasard parmi les recettes réellement enregistrées par l'utilisateur
   *  (DATA.state.roMazavaRecipes — liste vide par défaut, remplie manuellement depuis Paramètres >
   *  Ro mazava). Retourne null si l'utilisateur n'en a encore ajouté aucun : on n'invente jamais de
   *  Ro mazava fictif, le programme laisse alors ce complément vide plutôt que d'inventer. */
  pickRoMazava(avoidIds) {
    const list = (DATA.state.roMazavaRecipes || []).filter((r) => r.dishSubCategory !== "ro_matsatso");
    return this.pickRecipe(list, avoidIds || new Set());
  },

  /** Un Lasary/Achard au hasard parmi les vraies recettes de l'utilisateur (dishCategory==="achard"). */
  pickLasary(avoidIds) {
    const list = (DATA.state.recipes || []).filter((r) => r.dishCategory === "achard");
    return this.pickRecipe(list, avoidIds || new Set());
  },

  /** Construit un créneau { mode, recipeId, sideDishType, sideRecipeId } pour un jour/moment donné.
   *  sideDishType vaut "ro_mazava" | "lasary" | null — sideRecipeId pointe alors vers la recette
   *  d'accompagnement choisie (ou reste null si l'utilisateur n'en a aucune disponible). */
  buildSlot(day, usedIds) {
    let mode;
    if (day === "alatsinainy" || day === "sabotsy") {
      mode = "RONY";
    } else if (day === "alahady") {
      mode = null; // Dimanche : pas de contrainte de mode, uniquement "délicieux" (voir plus bas)
    } else {
      mode = this.ROTATION_MODES[Math.floor(Math.random() * this.ROTATION_MODES.length)];
    }

    let recipe = null;
    if (day === "alahady") {
      // Dimanche : repas de fête — viande, poisson, volaille ou mix, tous modes de cuisson confondus.
      const feastPool = (DATA.state.recipes || []).filter((r) => r.dishCategory !== "achard" && this.isMeatBased(r));
      recipe = this.pickRecipe(feastPool, usedIds);
      mode = recipe ? (recipe.cookingMethod || recipe.type) : null;
    } else if (day === "sabotsy") {
      // Samedi : RONY obligatoire, sans viande.
      const pool = this.recipesForMode("RONY").filter((r) => this.isMeatFree(r));
      recipe = this.pickRecipe(pool, usedIds);
    } else {
      const pool = this.recipesForMode(mode);
      recipe = this.pickRecipe(pool, usedIds);
      // Si la rotation aléatoire tombe sur un mode sans aucune recette disponible chez
      // l'utilisateur, on retente avec RONY (toujours requis en dernier recours pour Lundi de
      // toute façon) plutôt que de laisser un créneau vide alors que d'autres modes ont des plats.
      if (!recipe) {
        for (const fallbackMode of this.ROTATION_MODES) {
          const fallbackPool = this.recipesForMode(fallbackMode);
          recipe = this.pickRecipe(fallbackPool, usedIds);
          if (recipe) { mode = fallbackMode; break; }
        }
      }
    }
    ...
    // Complément automatique : Ro mazava pour RITRA viande/poisson/fruits de mer, Ro mazava OU
    // Lasary pour ENDASINA (quel que soit l'ingrédient).
    let sideDishType = null;
    let sideRecipe = null;
    const effectiveMode = recipe ? (recipe.cookingMethod || recipe.type) : mode;
    if (recipe && effectiveMode === "RITRA" && this.isMeatBased(recipe)) {
      sideRecipe = this.pickRoMazava(usedIds);
      if (sideRecipe) sideDishType = "ro_mazava";
    } else if (recipe && effectiveMode === "ENDASINA") {
      // Alterne Ro mazava / Lasary selon disponibilité réelle, en essayant Ro mazava en priorité.
      sideRecipe = this.pickRoMazava(usedIds);
      sideDishType = sideRecipe ? "ro_mazava" : null;
      if (!sideRecipe) {
        sideRecipe = this.pickLasary(usedIds);
        sideDishType = sideRecipe ? "lasary" : null;
      }
    }
    if (sideRecipe) usedIds.add(sideRecipe.id);

    return {
      mode: effectiveMode || null,
      recipeId: recipe ? recipe.id : null,
      sideDishType,
      sideRecipeId: sideRecipe ? sideRecipe.id : null,
    };
  },

  /** Génère (ou régénère) le programme complet de la semaine en cours et le persiste.
   *  usedIds est partagé sur toute la semaine pour limiter les répétitions d'un jour à l'autre,
   *  sans jamais bloquer un créneau si un mode/catégorie ne contient qu'une seule recette. */
  generate() {
    if (!DATA.state.mealPlan) DATA.state.mealPlan = { active: true, weekKey: null, days: {} };
    const usedIds = new Set();
    const days = {};
    this.DAYS.forEach((day) => {
      days[day] = {};
      this.MOMENTS.forEach((moment) => {
        days[day][moment] = this.buildSlot(day, usedIds);
      });
    });
    DATA.state.mealPlan.days = days;
    DATA.state.mealPlan.weekKey = this.currentWeekKey();
    DATA.state.mealPlan.generatedAt = new Date().toISOString();
    DATA.saveData();
    return DATA.state.mealPlan;
  },

  /** Régénère automatiquement si nécessaire (nouvelle semaine ISO) — à appeler au chargement de
   *  l'accueil. Ne fait rien si le programme n'a jamais été activé par l'utilisateur. */
  ensureFresh() {
    if (this.needsRefresh()) this.generate();
  },

  /** Résout un créneau { mode, recipeId, sideDishType, sideRecipeId } en objets recette réels,
   *  prêts pour l'affichage (recipe/sideRecipe peuvent être null si la recette a été supprimée
   *  depuis la génération, ou si aucun complément n'a été trouvé). */
  resolveSlot(slot) {
    if (!slot) return null;
    const recipe = slot.recipeId ? DATA.getRecipe(slot.recipeId) : null;
    const sideRecipe = slot.sideRecipeId ? DATA.getRecipe(slot.sideRecipeId) : null;
    return { ...slot, recipe, sideRecipe };
  },
};
...
window.RECO = RECO;
window.MENU_PLANNER = MENU_PLANNER;
