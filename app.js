...
const CALC = {
  ...
  toBaseQty(qty, unit, productUnit) {
    // Unités malagasy standardisées : l'unité affichée à l'utilisateur reste "fehezana"/"vilia"/
    // "tavoahangy"/"sotro" (voir UNIT_EQUIVALENTS), mais le moteur convertit d'abord vers
    // l'équivalence de référence (g ou ml) avant de continuer la conversion normale vers l'unité
    // de base du produit (kg ou l). Ex: 0.5 fehezana -> 0.5*150=75 g -> 75/1000=0.075 kg.
    if (UNIT_EQUIVALENTS[unit]) {
      const equiv = UNIT_EQUIVALENTS[unit];
      return this.toBaseQty(qty * equiv.qty, equiv.unit, productUnit);
    }
   ...
   */
  calculateRecipeCost(recipe, pantry, people) {
    const lines = [];
    ...
    recipe.ingredients.forEach((ing) => {
      ...
      const consumptionCost = this.priceForQty(product, ing.qty, ing.unit, people);
      totalConsumption += consumptionCost;
      if (isForfait) forfaitConsumption += consumptionCost; else scalableConsumption += consumptionCost;
    ...
    return { lines, totalConsumption, totalToBuy, forfaitConsumption, forfaitToBuy, scalableConsumption, scalableToBuy, haveList, buyList };
  },
...
const RECO = {
  ...
  getRecommendations(filters) {
    ...
    const scored = candidates.map((recipe) => {
      const cost = CALC.calculateRecipeCost(recipe, pantry, filters.people);
      // Le forfait (voatabia/tongolo, etc.) est déjà calculé pour filters.people — on ne le
      // remet pas à l'échelle. Seule la part "normale" (prorata) est mise à l'échelle par rapport
      // à recipe.servings.
      let scaledToBuy = this.scaleCostForServings(cost.scalableToBuy, recipe.servings, filters.people) + cost.forfaitToBuy;
      let scaledConsumption = this.scaleCostForServings(cost.scalableConsumption, recipe.servings, filters.people) + cost.forfaitConsumption;
      let displayPeople = filters.people;
      let budgetAdjusted = false;
      let overBudget = filters.budget ? scaledToBuy > filters.budget : false;

      // Ajustement automatique au budget : au lieu d'afficher un avertissement "budget
      // insuffisant", on réduit discrètement le nombre de personnes pour rentrer dans le budget
      // donné, tant qu'un nombre >= 1 personne le permet. Le bandeau d'avertissement classique
      // reste uniquement en dernier recours, si même 1 personne dépasse le budget.
      if (overBudget && filters.people > 1) {
  ...
  findMaxPeopleForBudget(recipe, pantry, budget, requestedPeople) {
    for (let p = requestedPeople - 1; p >= 1; p--) {
      ...
      const scaledToBuy = this.scaleCostForServings(cost.scalableToBuy, recipe.servings, p) + cost.forfaitToBuy;
      if (scaledToBuy <= budget) {
        const scaledConsumption = this.scaleCostForServings(cost.scalableConsumption, recipe.servings, p) + cost.forfaitConsumption;
