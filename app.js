...
/** Deep clone compatible avec tous les navigateurs (évite toute dépendance à structuredClone) */
...
const DATA = {
  ...
  loadData() {
    ...
    if (raw) {
      try {
        ...
        this.state.products = this.state.products || deepClone(DEFAULT_PRODUCTS);
        this.state.recipes = this.state.recipes || deepClone(DEFAULT_RECIPES);
        this.state.compatibility = this.state.compatibility || deepClone(DEFAULT_COMPATIBILITY);
        this.state.pantry = this.state.pantry || [];
        this.state.history = this.state.history || [];
        this.state.profile = this.state.profile || deepClone(DEFAULT_PROFILE);
        this.state.settings = this.state.settings || deepClone(DEFAULT_SETTINGS);
  ...
  resetToDemo(persist = true) {
    this.state = {
      ...
      recipes: deepClone(DEFAULT_RECIPES),
      compatibility: deepClone(DEFAULT_COMPATIBILITY),
      pantry: deepClone(DEFAULT_PANTRY),
      history: deepClone(DEFAULT_HISTORY),
...
const CALC = {
  ...
  toBaseQty(qty, unit, productUnit) {
    // normalise vers l'unité de base du produit (kg, l, piece, bunch)
    if (productUnit === "kg") {
    ...
    return qty; // piece / bunch : pas de sous-unité
  },
