...
const UI = {
  ...
  renderHome() {
    ...
    // Diaporama : toutes les recettes ayant une photo réellement importée (recipe.photoImage,
   ...
   *  bindHomeSlideshow gère la rotation automatique (20s) ; les points en bas permettent aussi de
   *  naviguer manuellement (chaque clic relance le minuteur de 20s pour ne pas sauter aussitôt). */
  ...
  bindHomeSlideshow(recipes) {
    ...
    const startTimer = () => {
      if (this._homeSlideshowTimer) clearInterval(this._homeSlideshowTimer);
      this._homeSlideshowTimer = setInterval(() => showSlide(index + 1), 20000);
    };
