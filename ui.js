...
const UI = {
  ...
  LAOKA_DASHBOARD_FRAMES: [
    ...
    { key: "saosy", icon: "🥣", label: "Saosy", kind: "dishSubCategory", value: "saosy" },
    { key: "ro_mazava", icon: "🍵", label: "Ro mazava", kind: "dishSubCategory", value: "ro_mazava" },
    { key: "ro_matsatso", icon: "🌿", label: "Ro matsatso", kind: "dishSubCategory", value: "ro_matsatso" },
    { key: "legioma", icon: "🥬", label: "Legioma", kind: "ingredientGroup", value: "legioma" },
    { key: "hena", icon: "🥩", label: "Hena", kind: "ingredientGroup", value: "hena" },
    { key: "trondro", icon: "🐟", label: "Trondro", kind: "ingredientGroup", value: "trondro" },
    { key: "vorona", icon: "🐔", label: "Vorona", kind: "ingredientGroup", value: "vorona" },
    { key: "fruits_de_mer", icon: "🦐", label: "Fruits de mer", kind: "ingredientGroup", value: "fruits_de_mer" },
    { key: "ambany_tanana", icon: "🍖", label: "Ambany tanana", kind: "ingredientGroup", value: "ambany_tanana" },
    { key: "atody", icon: "🥚", label: "Atody", kind: "ingredientGroup", value: "atody" },
    { key: "bebe", icon: "🍼", label: "Bébé", kind: "dishSubCategory", value: "bebe" },
    { key: "gouter", icon: "🌮", label: "Goûter", kind: "dishSubCategory", value: "gouter" },
    { key: "vegetarien", icon: "🌱", label: "Végétarien", kind: "diet", value: "vegetarien" },
    { key: "fety", icon: "🎉", label: "Fety", kind: "special", value: "fety" },
  ],
  ...
  recipesForFrame(frame) {
    ...
    if (frame.kind === "ingredientGroup") {
      const map = {
        legioma: ["legume", "legumes", "legioma", "anana", "voamaina"],
        hena: ["hena", "viande", "meat"],
        trondro: ["trondro", "poisson", "fish"],
        vorona: ["vorona", "volaille", "poultry"],
        fruits_de_mer: ["fruits_de_mer", "fruits-de-mer", "seafood", "fruit_de_mer"],
        ambany_tanana: ["ambany_tanana", "ambany-tanana", "abats", "partie_particuliere"],
        atody: ["atody", "oeuf", "egg"]
      };
      const wanted = map[frame.value] || [frame.value];
      return recipes.filter((r) => wanted.includes(String(r.ingredientType || "").toLowerCase()) || wanted.includes(String(r.foodRole || "").toLowerCase()));
    }
    if (frame.kind === "diet") {
      return recipes.filter((r) => Array.isArray(r.diet) && r.diet.some((d) => String(d).toLowerCase() === frame.value));
    }
    if (frame.kind === "special") {
      return recipes.filter((r) => Array.isArray(r.tags) && r.tags.includes(frame.value) || Array.isArray(r.moment) && r.moment.includes(frame.value));
    }
  ...
  openSearchForm(preset = {}) {
    ...
    document.getElementById("searchForm").addEventListener("submit", (e) => {
      ...
      this.state.searchFilters = { budget, people, mealTime: selectedMoment, laokaType: selectedType, trondroState: isFishKind(selectedType) ? selectedTrondroState : null, dishSubCategory: selectedDishSubCategory, includeAchard };
      this.closeModal();
      this.runSearch();
    });
  },
