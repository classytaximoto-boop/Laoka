...
const UI = {
  state: {
    currentScreen: "home",
    searchFilters: { budget: 3000, people: 4, mealTime: null, laokaType: null, dishSubCategory: null, includeAchard: false },
    lastResults: [],
  },
  ...
  openSearchForm(preset = {}) {
    ...
    const laokaTypes = ["RITRA", "RONY", "KETSAKETSA", "KIENDY", "MAINA", "SAOSY", "LASOPY", "LEGUMES", "POISSON", "VIANDE", "ATODY"];
      ...
      <form id="searchForm" class="form">
        ...
        <div class="field">
          ...
          <div class="choice-grid" id="fDishSubCategoryWrap">
            <button type="button" class="choice-chip ${!f.dishSubCategory?"active":""}" data-value="">Rehetra</button>
            ${["laoka_classique","rony","ritra","ketsaketsa","kiendy","voamaina","voalena","ambany_tanana"].map((c) => `<button type="button" class="choice-chip ${f.dishSubCategory===c?"active":""}" data-value="${c}">${DISH_CATEGORY_LABELS[c]}</button>`).join("")}
          </div>
        </div>
        ...
        <div class="field">
          ...
          <div class="choice-grid" id="fLaokaTypeWrap">
            ${laokaTypes.map((t) => `<button type="button" class="choice-chip ${f.laokaType===t?"active":""}" data-value="${t}">${t}</button>`).join("")}
          </div>
        </div>
    ...
    document.getElementById("searchForm").addEventListener("submit", (e) => {
      ...
      this.state.searchFilters = { budget, people, mealTime: selectedMoment, laokaType: selectedType, dishSubCategory: selectedDishSubCategory, includeAchard };
      this.closeModal();
      this.runSearch();
    });
  },
