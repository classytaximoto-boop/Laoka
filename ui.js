...
const UI = {
  ...
  /** Toute photo réelle de laoka (recipe.photoImageId) est stockée dans IndexedDB (PHOTO_STORE,
   *  app.js), jamais dans le HTML directement — le rendu des cartes marque simplement l'élément
   *  avec data-photo-id, et cette fonction, appelée juste après l'insertion dans le DOM, charge
   *  chaque photo de façon async et l'injecte une fois prête (<img> ou fond, selon le conteneur).
   *  `root` (optionnel) : conteneur dans lequel chercher [data-photo-id] — document entier si omis. */
  hydratePhotoElements(root) {
    (root || document).querySelectorAll("[data-photo-id]").forEach((el) => {
      const id = el.dataset.photoId;
      if (!id) return;
      PHOTO_STORE.get(id).then((dataUrl) => {
        if (!dataUrl) return;
        if (el.tagName === "IMG") {
          el.src = dataUrl;
        } else {
  ...
  renderHome() {
    ...
    // Carrousel "Laoka anio" : jusqu'à 5 suggestions, qui tournent automatiquement (1 min) — voir
    // bindHomeSuggestionRotation. getTodaySuggestion() (singulier) reste utilisée telle quelle
    // ailleurs si besoin ; ici on prend la variante multiple dédiée à ce carrousel.
    const suggestions = RECO.getTodaySuggestions(5);
    const soonItems = STOCK.getItemsToUseSoon();
    // Diaporama photo : toutes les recettes ayant une photo réellement importée (recipe.photoImageId,
    // voir ui-laoka-form.js / PHOTO_STORE dans app.js). Si aucune recette n'a de photo, la section
    // est simplement absente — comportement historique de l'accueil inchangé dans ce cas.
    const slideshowRecipes = (DATA.state.recipes || []).filter((r) => r.photoImageId);
    ...
    el.innerHTML = `
      ...
      ${slideshowRecipes.length ? this.homeSlideshowHTML(slideshowRecipes) : ""}
      ...
      <section class="section fade-in">
        ...
        <div id="homeSuggestionCard">
          ${suggestions.length ? this.recipeCardHTML(suggestions[0], { compact: true }) : `<p class="muted">Tsy misy tolotra amin'izao fotoana izao.</p>`}
        </div>
      </section>

      ${this.mealPlanWidgetHTML()}

      <p class="home-greeting fade-in" id="homeGreeting"></p>
    `;
    ...
    if (suggestions.length) this.bindHomeSuggestionRotation(suggestions);
    if (slideshowRecipes.length) this.bindHomeSlideshow(slideshowRecipes);
    this.hydratePhotoElements(el);
    this.bindMealPlanWidget();
  },

  /* ------------------- PROGRAMME DE LAOKA SUR 7 JOURS ------------------- */
  /** Widget accueil (bas de page) : programme hebdomadaire Midi/Soir synchronisé automatiquement
   *  avec les recettes de l'utilisateur (voir MENU_PLANNER dans app.js). Régénère tout seul la
   *  semaine si l'utilisateur revient sur une nouvelle semaine ISO (programme déjà actif). */
  mealPlanWidgetHTML() {
    MENU_PLANNER.ensureFresh();
    const plan = DATA.state.mealPlan;
    const active = plan && plan.active;

    return `
      <section class="section fade-in" id="mealPlanSection">
        <h2 class="section-title">📅 Fandaharana laoka 7 andro</h2>
        <div class="card">
          ${
            !active
              ? `<p class="muted">Mamorona fandaharana laoka isan-kerinandro (atoandro + hariva), mifanaraka avy hatrany amin'ny lisitrao. Alatsinainy sy Sabotsy dia RONY foana ; Sabotsy tsy misy hena ; Alahady laoka tsara indrindra.</p>
                 <button class="btn btn-primary btn-block" id="btnMealPlanActivate">▶️ Alefaso ny fandaharana</button>`
              : `<div class="meal-plan-header-row">
                   <span class="muted small">Herinandro ${plan.weekKey || ""}</span>
                   <div class="meal-plan-header-actions">
                     <button class="btn btn-tertiary btn-sm" id="btnMealPlanRefresh">🔄 Avaozo</button>
                     <button class="btn btn-tertiary btn-sm" id="btnMealPlanStop">⏹️ Ajanony</button>
                   </div>
                 </div>
                 <div class="meal-plan-grid">${this.mealPlanDaysHTML(plan)}</div>`
          }
        </div>
      </section>
    `;
  ...
  mealPlanDaysHTML(plan) {
    return MENU_PLANNER.DAYS.map((day) => {
      const dayData = plan.days[day] || {};
      return `
        <div class="meal-plan-day">
          <div class="meal-plan-day-label">${MENU_PLANNER.DAY_LABELS[day]}</div>
          <div class="meal-plan-slots">
            ${MENU_PLANNER.MOMENTS.map((moment) => {
              const resolved = MENU_PLANNER.resolveSlot(dayData[moment]);
              return this.mealPlanSlotHTML(day, moment, resolved);
            }).join("")}
          </div>
        </div>
      `;
  ...
  mealPlanSlotHTML(day, moment, resolved) {
    if (!resolved || !resolved.recipe) {
      return `
        <div class="meal-plan-slot">
          <span class="meal-plan-moment">${MENU_PLANNER.MOMENT_LABELS[moment]}</span>
          <span class="meal-plan-empty muted">— (mbola tsy ampy laoka amin'ity karazany ity)</span>
        </div>`;
    }
    const sideLabel = resolved.sideRecipe
      ? `<span class="meal-plan-side">+ ${resolved.sideDishType === "ro_mazava" ? "🍲" : "🥗"} ${resolved.sideRecipe.name}</span>`
      : "";
    return `
      <div class="meal-plan-slot" data-day="${day}" data-moment="${moment}" data-recipe-id="${resolved.recipe.id}">
        <span class="meal-plan-moment">${MENU_PLANNER.MOMENT_LABELS[moment]}</span>
        <span class="meal-plan-recipe">${resolved.recipe.photo || ""} ${resolved.recipe.name}</span>
        ${sideLabel}
      </div>
    `;
  },
  ...
  bindMealPlanWidget() {
    const activateBtn = document.getElementById("btnMealPlanActivate");
    if (activateBtn) {
      activateBtn.addEventListener("click", () => {
        MENU_PLANNER.activate();
        this.refreshMealPlanWidget();
      });
    }
    const refreshBtn = document.getElementById("btnMealPlanRefresh");
    if (refreshBtn) {
      refreshBtn.addEventListener("click", () => {
        MENU_PLANNER.generate();
        this.refreshMealPlanWidget();
      });
    }
    const stopBtn = document.getElementById("btnMealPlanStop");
    if (stopBtn) {
      stopBtn.addEventListener("click", () => {
        MENU_PLANNER.deactivate();
        this.refreshMealPlanWidget();
      });
    }
    document.querySelectorAll(".meal-plan-slot[data-recipe-id]").forEach((slot) => {
      slot.addEventListener("click", () => {
        const recipe = DATA.getRecipe(slot.dataset.recipeId);
        if (recipe) this.openRecipeDetail(this.wrapRecipeAsResult(recipe));
      });
    });
  },

  /** Redessine uniquement la section du programme (pas tout l'accueil), pour ne pas relancer les
   *  autres carrousels/minuteurs de renderHome() à chaque clic sur Activer/Actualiser/Arrêter. */
  refreshMealPlanWidget() {
    const section = document.getElementById("mealPlanSection");
    if (!section) return;
    section.outerHTML = this.mealPlanWidgetHTML();
    this.bindMealPlanWidget();
  },

  /** Phrase du bas de l'accueil, selon l'heure locale de l'appareil : message "journée" avant 18h,
   *  message "soirée" à partir de 18h. Pas de minuteur dédié ici — la formule ne dépend que de
   *  l'heure au moment du rendu, donc elle se met à jour toute seule à chaque renderHome() (ex.
   *  déjà rafraîchi par bindHomeSuggestionRotation toutes les minutes). */
  renderHomeGreeting() {
    const el = document.getElementById("homeGreeting");
    if (!el) return;
    const hour = new Date().getHours();
    el.textContent = hour >= 18 || hour < 4 ? "Mirary hariva finaritra." : "Mirary tontolo andro sambatra ho anao.";
  },

  /** Fait tourner la carte "Laoka anio" parmi plusieurs suggestions (RECO.getTodaySuggestions),
   *  toutes les 60 secondes — ne redessine que #homeSuggestionCard, pas toute la page. Réutilise
   *  au passage l'appel pour rafraîchir la phrase du bas (renderHomeGreeting), afin qu'un accueil
   *  laissé ouvert à cheval sur 18h bascule tout seul sur le message du soir. */
  bindHomeSuggestionRotation(suggestions) {
    if (this._homeSuggestionTimer) clearInterval(this._homeSuggestionTimer);
    const wrap = document.getElementById("homeSuggestionCard");
    if (!wrap || suggestions.length < 2) return; // rien à faire tourner s'il n'y a qu'une seule suggestion
    let index = 0;

    const bindCardButtons = (suggestion) => {
      const viewBtn = wrap.querySelector(".recipe-card [data-action='view']");
      if (viewBtn) viewBtn.addEventListener("click", () => this.openRecipeDetail(suggestion));
    };
    bindCardButtons(suggestions[0]);

    this._homeSuggestionTimer = setInterval(() => {
      index = (index + 1) % suggestions.length;
      wrap.innerHTML = this.recipeCardHTML(suggestions[index], { compact: true });
      bindCardButtons(suggestions[index]);
      this.renderHomeGreeting();
      this.hydratePhotoElements(wrap);
    }, 60000);
  },

  /** Diaporama accueil : une slide par recette photographiée, titre en overlay en bas de l'image.
   *  bindHomeSlideshow gère la rotation automatique (15s) ; les points en bas permettent aussi de
   *  naviguer manuellement (chaque clic relance le minuteur de 15s pour ne pas sauter aussitôt). */
  homeSlideshowHTML(recipes) {
    return `
      <div class="home-slideshow card fade-in" id="homeSlideshow">
        <div class="home-slideshow-track">
          ${recipes.map((r, i) => `
            <div class="home-slideshow-slide ${i === 0 ? "active" : ""}" data-index="${i}" data-id="${r.id}">
              <img data-photo-id="${r.photoImageId}" alt="${r.name}">
              <div class="home-slideshow-caption">${r.name}</div>
            </div>
          `).join("")}
        </div>
        ${recipes.length > 1 ? `
          <div class="home-slideshow-dots">
            ${recipes.map((_, i) => `<button class="home-slideshow-dot ${i === 0 ? "active" : ""}" data-index="${i}" aria-label="Sary ${i + 1}"></button>`).join("")}
          </div>
        ` : ""}
      </div>
    `;
  ...
  bindHomeSlideshow(recipes) {
    // Un seul diaporama actif à la fois : on nettoie tout minuteur laissé par un précédent
    // renderHome() (navigation aller-retour vers l'accueil) avant d'en démarrer un nouveau.
    if (this._homeSlideshowTimer) clearInterval(this._homeSlideshowTimer);
    const wrap = document.getElementById("homeSlideshow");
    if (!wrap) return;
    let index = 0;
    const slides = wrap.querySelectorAll(".home-slideshow-slide");
    const dots = wrap.querySelectorAll(".home-slideshow-dot");

    const showSlide = (i) => {
      index = (i + recipes.length) % recipes.length;
    ...
    const startTimer = () => {
      if (this._homeSlideshowTimer) clearInterval(this._homeSlideshowTimer);
      this._homeSlideshowTimer = setInterval(() => showSlide(index + 1), 15000);
    };
    ...
    // Tap sur la photo elle-même : ouvre la fiche détail de ce laoka, comme les autres cartes.
    slides.forEach((slide) => {
      slide.addEventListener("click", () => {
        const recipe = recipes.find((r) => r.id === slide.dataset.id);
        if (recipe) this.openRecipeDetail(this.wrapRecipeAsResult(recipe));
      });
    });
  ...
  LAOKA_DASHBOARD_FRAMES: [
    ...
    { key: "hanikotrana", icon: "🍠", label: "Hanikotrana", kind: "dishSubCategory", value: "hanikotrana" },
    { key: "lasary", icon: "🥗", label: "Lasary", kind: "dishSubCategory", value: "lasary" },
  ],
  ...
  dashboardMiniCardHTML(recipe) {
    ...
    return `
      <div class="laoka-mini-card" data-id="${recipe.id}" data-action="view">
        <span class="laoka-mini-photo">${recipe.photoImageId ? `<img data-photo-id="${recipe.photoImageId}" alt="${recipe.name}">` : (recipe.photo || "🍽️")}</span>
   ...
   *  recipe.servings (comportement historique, aucun scaling). Le mode de portion réutilise le
   *  dernier choisi dans le formulaire de recherche (searchFilters.portionMode), à défaut "normal". */
  wrapRecipeAsResult(recipe, people) {
    const targetPeople = people || recipe.servings;
    const portionMode = (this.state.searchFilters && this.state.searchFilters.portionMode) || "normal";
    const cost = CALC.calculateRecipeCost(recipe, DATA.state.pantry, targetPeople, portionMode);
    const scaledToBuy = RECO.scaleCostForServings(cost.scalableToBuy, recipe.servings, targetPeople, recipe) + cost.forfaitToBuy + cost.portionAdjustedToBuy;
    const scaledConsumption = RECO.scaleCostForServings(cost.scalableConsumption, recipe.servings, targetPeople, recipe) + cost.forfaitConsumption + cost.portionAdjustedConsumption;
  ...
  openSearchForm(preset = {}) {
    ...
    const html = `
      ...
      <form id="searchForm" class="form">
        ...
        <div class="field">
          <label>Habetsahan'ny hena/trondro/holatra</label>
          <p class="muted small">Mifantina hoe hoatrinona ny henan-kisoa, akoho, trondro, fruits de mer na holatra ho isaky ny olona iray — tsy misy fiovana amin'ny legioma/anana.</p>
          <div class="choice-grid" id="fPortionMode">
            ${PORTION_MODES.map((m) => `<button type="button" class="choice-chip ${(f.portionMode || "normal") === m ? "active" : ""}" data-value="${m}">${PORTION_MODE_LABELS[m]}</button>`).join("")}
          </div>
        </div>
    ...
    let selectedPortionMode = f.portionMode || "normal";
    document.querySelectorAll("#fPortionMode .choice-chip").forEach((btn) => {
      btn.addEventListener("click", () => {
        document.querySelectorAll("#fPortionMode .choice-chip").forEach((b) => b.classList.remove("active"));
        btn.classList.add("active");
        selectedPortionMode = btn.dataset.value;
      });
    });
    ...
    document.getElementById("searchForm").addEventListener("submit", (e) => {
      ...
      this.state.searchFilters = { budget, people, mealTime: selectedMoment, laokaType: selectedType, trondroState: isFishKind(selectedType) ? selectedTrondroState : null, dishSubCategory: selectedDishSubCategory, includeAchard, searchQuery, portionMode: selectedPortionMode };
      this.closeModal();
      this.runSearch();
    });
  },
  ...
  renderResultsScreen(results) {
    ...
    list.querySelectorAll(".recipe-card").forEach((card, idx) => {
      ...
      this.bindCardSwipeToKitchenLive(card, r);
    });
    this.hydratePhotoElements(list);
  },

  /** Swipe rapide droite→gauche sur une carte de résultat (liste) : ouvre directement l'écran
   *  Mahandro (guide pas-à-pas avec chrono + température) pour ce laoka, sans passer par le
   *  bouton "Mahandro" de la fiche détail. Seuil et tolérance verticale calqués sur le swipe déjà
   *  utilisé dans ui-mahandro.js pour rester cohérent, mais sans effet de drag visuel : ici on ne
   *  fait que détecter le geste et déclencher MAHANDRO_UI.open au relâchement. */
  /** Swipe rapide droite→gauche sur une carte de résultat (liste) : ouvre l'écran « Ao an-dakozia »
   *  (mode cuisson en direct — chrono live, température, feu, contenu marmite, voir
   *  ui-kitchen-live.js), distinct du bouton « Mahandro » de la fiche détail qui reste le guide
   *  pas-à-pas classique par étapes. */
  bindCardSwipeToKitchenLive(card, result) {
    ...
    card.addEventListener("touchmove", (e) => {
      ...
      // Une fois qu'on sait que c'est un swipe horizontal, on empêche le scroll de la liste
      // de "voler" le geste avant touchend.
      if (axisLocked === "x") e.preventDefault();
    }, { passive: false });

    card.addEventListener("touchend", (e) => {
      if (startX == null || axisLocked !== "x") {
        startX = null; startY = null; axisLocked = null;
        return;
      }
      const dx = e.changedTouches[0].clientX - startX;
      startX = null; startY = null; axisLocked = null;
      if (dx < -SWIPE_THRESHOLD) KITCHEN_LIVE_UI.open(result);
    }, { passive: true });
  ...
  recipeCardHTML(result, opts = {}) {
    ...
    return `
      <div class="card recipe-card fade-in">
        ...
        ${budgetAdjusted ? `<p class="muted small budget-adjusted-note">⚠️ Nangatahinao ${requestedPeople} olona, saingy ${people} olona ihany no tratra amin'ny budget-nao.</p>` : ""}
        ${
          overBudget
            ? `<div class="warning-banner">${
                DATA.state.settings.tone === "basy_vava"
                  ? "Zany ve ho ampy ry mahidy ah! Tsy indroa ny miaina🙄"
                  : `Tsy ampy ${CALC.formatAr(Math.abs(budgetRemaining))} ny budget-nao amin'ity laoka ity.`
              }</div>`
            : ""
        }
        ...
        ${recipe.photoImageId ? `<img class="recipe-photo-import" data-photo-id="${recipe.photoImageId}" alt="${recipe.name}">` : ""}
      </div>
    `;
  },
  ...
  openRecipeDetail(result) {
    ...
    const html = `
      ...
      <div class="recipe-detail">
        ...
        ${result.budgetAdjusted ? `<p class="muted small budget-adjusted-note">⚠️ Nangatahinao ${result.requestedPeople} olona, saingy ${displayPeople} olona ihany no tratra amin'ny budget-nao (${CALC.formatAr(this.state.searchFilters.budget || 0)}).</p>` : ""}
        ...
        <table class="price-table">
          ${cost.lines.map((l) => `
            <tr>
              ...
              <td>${(l.isForfait || l.isPortionAdjustable) ? l.qty : Math.round(l.qty * scaleFactor * 1000) / 1000} ${l.unit}</td>
              <td>${CALC.formatAr((l.isForfait || l.isPortionAdjustable) ? l.consumptionCost : Math.round(l.consumptionCost * scaleFactor))}</td>
            </tr>
          `).join("")}
          <tr class="total-row">
        ...
        ${recipe.photoImageId ? `<img class="recipe-photo-import" data-photo-id="${recipe.photoImageId}" alt="${recipe.name}" style="margin-top:14px;">` : ""}
      </div>
    `;
    this.openModal(html);
    this.hydratePhotoElements(document.getElementById("modalOverlay") || document);
