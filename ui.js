...
const UI = {
  state: {
    currentScreen: "home",
    searchFilters: { budget: 3000, people: 4, mealTime: null, laokaType: null, includeAchard: false },
    lastResults: [],
  },
  ...
  openSearchForm(preset = {}) {
    ...
    document.querySelectorAll("#searchForm .choice-grid .choice-chip").forEach((btn) => {
      btn.addEventListener("click", () => {
        const already = btn.classList.contains("active");
        document.querySelectorAll("#searchForm .choice-grid .choice-chip").forEach((b) => b.classList.remove("active"));
        if (!already) {
          btn.classList.add("active");
    ...
    document.getElementById("searchForm").addEventListener("submit", (e) => {
      ...
      this.state.searchFilters = { budget, people, mealTime: selectedMoment, laokaType: selectedType, includeAchard };
      this.closeModal();
      this.runSearch();
    });
  },
