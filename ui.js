...
const UI = {
  ...
  renderHome() {
    ...
    // Diaporama photo : toutes les recettes ayant une photo réellement importée (recipe.photoImage,
    // voir ui-laoka-form.js). Si aucune recette n'a de photo, la section n'est simplement pas
    // affichée — comportement historique de l'accueil inchangé dans ce cas.
    const slideshowRecipes = (DATA.state.recipes || []).filter((r) => r.photoImage);
        ...
        <div class="home-slideshow-track">
          ${recipes.map((r, i) => `
            <div class="home-slideshow-slide ${i === 0 ? "active" : ""}" data-index="${i}" data-id="${r.id}">
              <img src="${r.photoImage}" alt="${r.name}">
  ...
  dashboardMiniCardHTML(recipe) {
    ...
    return `
      <div class="laoka-mini-card" data-id="${recipe.id}" data-action="view">
        <span class="laoka-mini-photo">${recipe.photoImage ? `<img src="${recipe.photoImage}" alt="${recipe.name}">` : (recipe.photo || "🍽️")}</span>
  ...
  recipeCardHTML(result, opts = {}) {
    ...
    return `
      <div class="card recipe-card fade-in">
        ...
        ${recipe.photoImage ? `<img class="recipe-photo-import" src="${recipe.photoImage}" alt="${recipe.name}">` : ""}
      </div>
    `;
  },
  ...
  openRecipeDetail(result) {
    ...
    const html = `
      ...
      <div class="recipe-detail">
        ...
        ${recipe.photoImage ? `<img class="recipe-photo-import" src="${recipe.photoImage}" alt="${recipe.name}" style="margin-top:14px;">` : ""}
      </div>
    `;
    this.openModal(html);
