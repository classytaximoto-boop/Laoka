...
const ADMIN = {
  ...
  /* ------------------- PRODUITS / PRIX ------------------- */
  renderProduits() {
    this.produitSearchQuery = this.produitSearchQuery || "";
    document.getElementById("adminBody").innerHTML = `
      ...
      <div class="field fade-in" style="margin-bottom:12px;">
        <label for="produitSearchInput">Rechercher un produit (ex : akoho, legioma...)</label>
        <input type="text" id="produitSearchInput" placeholder="Karohy anarana na sokajy..." value="${this.produitSearchQuery}">
      </div>
    ...
    const searchInput = document.getElementById("produitSearchInput");
    searchInput.addEventListener("input", () => {
      this.produitSearchQuery = searchInput.value;
      this.renderProduitsTable();
    });
    this.renderProduitsTable();
  },

  /** Filtre DATA.state.products par produitSearchQuery : recherche insensible à la casse/accents
      sur le nom ET la catégorie (même pattern que renderRecettesTable). */
  renderProduitsTable() {
    const norm = (s) => (s || "").toString().normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase();
    const query = norm(this.produitSearchQuery);
    let products = DATA.state.products;
    if (query) {
      products = products.filter((p) => norm(p.name).includes(query) || norm(p.category).includes(query));
    }
    const tbody = document.getElementById("productsTableBody");
    if (!tbody) return;
    tbody.innerHTML = products.length === 0
      ? `<tr><td colspan="7"><p class="muted" style="padding:12px 0;">Tsy nahitana vokatra mifanaraka amin'ny "${this.produitSearchQuery}".</p></td></tr>`
      : products.map((p) => `
        <tr data-id="${p.id}">
          <td>${p.name}</td>
          <td>${p.category}</td>
    ...
    document.querySelectorAll("#productsTableBody tr").forEach((row) => {
      ...
      row.querySelector("[data-action='delete']").addEventListener("click", () => {
        DATA.state.products = DATA.state.products.filter((pr) => pr.id !== id);
        DATA.saveData();
        this.renderProduitsTable();
  ...
  openProductForm(existing) {
      ...
      <form id="prodForm" class="form">
        ...
        <div class="field">
          <label class="checkbox-label">
            <input type="checkbox" id="prForLasary" ${d.forLasary ? "checked" : ""}>
            Azo ampiasaina amin'ny Lasary (catégorie mifototra, tsy mitambatra amin'ny safidy etsy ambony)
          </label>
        </div>
        <div class="field">
    ...
    document.getElementById("prodForm").addEventListener("submit", (e) => {
      ...
      const forLasary = document.getElementById("prForLasary").checked;
